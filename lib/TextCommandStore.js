...
var TextCommandStore = (function() {
	...
	*/
	TextCommandStore.prototype.addCommand = function(gameId, commandJson, callback){
	...
	*/
	TextCommandStore.prototype.getCommands = function(gameId, callback){
		var commands = getGameCommands(gameId);
		callback(commands);
	};

	TextCommandStore.prototype.getCommandsSinceCommand = function(gameId, commandIndex, callback){
		this.getCommands(gameId, function(allCommands) {
			var commands = [];

			for(var i = commandIndex + 1; i < allCommands.length; i++)
			{
				commands.push(allCommands[i]);
			}
	...
	function getGameCommands(gameId)
	{
		...
		var commands = commandsFile.games[gameId];
		return commands || [];
	}
