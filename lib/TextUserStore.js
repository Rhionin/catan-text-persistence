...
var TextUserStore = (function() {
	...
	var users_file_path = path.resolve(path.join(__dirname, '..', 'data', 'users' + file_extension));

	function TextUserStore(file_path)
	{
		users_file_path = (file_path ? path.resolve(file_path) : users_file_path);

		// ensure the file exists and init it if not
		if (!fs.existsSync(users_file_path))
		{
			fs.openSync(users_file_path, 'a');
